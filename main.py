...
from models.build_models import EffiSegNetBN
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        'DataScienceBowl2018 training and evaluation script', add_help=False)

    # Dataset parameters
    parser.add_argument("--data_root", type=str, default='/mnt/d/DataScienceBowl2018',
                        help="path to DataScienceBowl2018 Dataset")
    ...
    parser.add_argument('--data_len', default=670, type=int,
    ...
    parser.add_argument("--val_print_freq", type=int, default=20)

    # Model parameters
    parser.add_argument('--model', default='efficientnet-b0', type=str, metavar='MODEL',
...
def main(args):
    ...
    model = EffiSegNetBN(nb_classes=args.nb_classes, model_name=args.model)
    ...
    for epoch in range(args.epochs):
        ...
        mean_loss, lr = train_one_epoch(model, trainloader, optimizer, device,
                                        epoch, loss_scaler, args.clip_grad, args.clip_mode, True,
                                        args.train_print_freq,
                                        writer, args)

        confmat, metric = evaluate(valloader, model, device, args.val_print_freq, writer, args)
        mean_iou = confmat.compute()[2].mean().item() * 100
        mean_iou = round(mean_iou, 2)
        all_f1, mean_f1 = metric.compute_f1()
        all_acc, mean_acc = metric.compute_pixel_acc()
    ...
    if args.predict and utils.is_main_process():
        model_pred = EffiSegNetBN(nb_classes=args.nb_classes, model_name=args.model)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'DataScienceBowl2018 training and evaluation script', parents=[get_args_parser()])
